...
class OrderAuthSale extends TaskRunnable
{
    ...
    private function share_money()
    {
        ...
        if(($shareSetting->level > 1 || ($shareSetting->level == 1 && $shareSetting->is_rebate == 0)) && $parentId1 > 0) {
            $this->set_money($shareMoney1, $parentId1, 1);
        }
        if(($shareSetting->level > 2 || ($shareSetting->level == 2 && $shareSetting->is_rebate == 0)) && $parentId2 > 0) {
            $this->set_money($shareMoney2, $parentId2, 2);
        }
        if($shareSetting->level >= 3 && $shareSetting->is_rebate == 0 && $parentId3 > 0) {
    ...
    private function transferToMch()
    {
        ...
        $accountMoney = floatval($mch->account_money) + floatval($this->order->pay_price * (1 - floatval($mch->transfer_rate) / 1000)) - $this->order->share_price;
